...
Dodatkowo w [tym artykule](https://danetyka.com/kontek-analiza-bledow/){target="_blank"} omawiam błędy popełnione przez dr. Krzysztofa Kontka w jego modelu detekcji anomalii.
...
Tutaj prezentuję [moje podejście](https://danetyka.com/model-detekcji-anomalii/){target="_blank"}, przy pomocy którego doszedłem do zupełnie innych wniosków: co najwyżej kilkadziesiąt komisji, które można uznać za anomalie, oraz symetryczny ich rozkład (podobna liczba z korzyścią dla Trzaskowskiego i Nawrockiego). I zacznę od uwagi ogólnej, którą można podsumować pytaniem: któremu modelowi zaufać? Ja oczywiście bardziej ufam swojemu, bo po pierwsze wiem, jak został skonstruowany, po drugie moim zdaniem jest obiektywnie lepszy. Natomiast to nie jest wystarczający argument, jeśli taki model miałby być w praktyce wykorzystywany. Moim zdaniem do tego trzeba podejść inaczej:
