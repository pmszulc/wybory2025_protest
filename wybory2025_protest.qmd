...
Jeden z dowodów opiera się na modelu opisanym na stronie defoliator.pl, który rzeczywiście może być użyty do wskazania komisji, w których mogło dojść do naruszeń. Jak jednak pokażę, nie pozwala na wyciąganie wniosków, że było ich wiele i głównie na korzyść jednego z kandydatów. Przy okazji omawianie tego modelu zaprezentuję alternatywne podejście do wykrywania takich komisji (anomalii).

W ostatniej części omawiam błędy popełnione w [artykule](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5296441){target="_blank"} dr. Krzysztofa Kontka. Są one tak poważne, że w mojej ocenie dyskwalifikują zastosowane tam podejście.
...
```{r}
#CairoPDF("DB/benford_Nupr.pdf", width = 7, height = 5)
wyb25 %>% 
  filter(Nupr_2 > 0) %>% 
  benford1("Nupr_2", "Liczba uprawnionych, druga tura 2025", limit = NA)
#dev.off()
# ggsave("DB/benford_Nupr.pdf", dpi = 150, bg = "white", width = 7, height = 5)
```
...
```{r}
#CairoPDF("DB/benford_Nawr.pdf", width = 7, height = 5)
benford1(wyb25, "Nawrocki_2", "Liczba głosów, Nawrocki, druga tura 2025", limit = NA)
#dev.off()
#ggsave("DB/benford_Nawr.pdf", dpi = 150, bg = "white", width = 7, height = 5)
```
...
```{r}
#CairoPDF("DB/benford_Duda.pdf", width = 7, height = 5)
benford1(wyb20, "Duda_2", "Liczba głosów, Duda, druga tura 2020", limit = NA)
#dev.off()
# ggsave("DB/benford_Duda.pdf", dpi = 150, bg = "white", width = 7, height = 5)
```
...
```{r}
wyb25 <- wyb25 %>% 
  # we wcześniejszej analizie nie uwzględniałem głosów nieważnych
  mutate(Frekwencja_1 = (N_1 + Nniew_1) / Nupr_1 * 100) %>% 
  mutate(Frekwencja_2 = (N_2 + Nniew_2) / Nupr_2 * 100) %>% 
...
```{r}
#CairoPDF("DB/ruski_ogon1.pdf", width = 7, height = 5)
wyb25 %>% 
  count(Frekwencja_2_zaokr) %>% 
  ggplot(aes(Frekwencja_2_zaokr, n)) +
  geom_point(alpha = 0.8) +
  geom_line()  +
  labs(x = "Frekwencja, druga tura", y = "Liczba komisji") +
  theme(
...
#dev.off()
#ggsave("DB/ruski_ogon1.pdf", dpi = 150, bg = "white", width = 7, height = 5)
```
...
```{r}
#CairoPDF("DB/ruski_ogon2.pdf", width = 7, height = 5)
wyb25 %>% 
  count(Frekwencja_2_zaokr) %>% 
  ...
  scale_y_continuous(limits = c(0, NA), breaks = seq(0, 120, by = 20))
#dev.off()
#ggsave("DB/ruski_ogon2.pdf", dpi = 150, bg = "white", width = 7, height = 5)
```
...
wyb25 %>% 
  filter(Frekwencja_2_zaokr >= 100, Typ_obwodu == "stały") %>% 
...
Poniżej komisje, dla których p-wartość (z korektą Bonferroniego) była mniejsza od 0,05. *Reszta_s* to reszta studentyzowana, przy pomocy której szukam danych odstających, *Różnica_pp* to różnica między oficjalnym wynikiem Trzaskowskiego a prognozowanym.
...
Dr. Kontek w swoim [artykule](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5296441){target="_blank"} używa dość ciekawej metody detekcji anomalii, opierającej się na założeniu, że w geograficznie bliskich komisjach wyborcy głosują podobnie, a przynajmniej brak jest komisji (w "lokalnej grupie"), która odstawałaby od reszty w sposób naturalny. Mimo że mam duży problem z samym tym założeniem, to przyjmijmy, że ma ono sens.

Autor zidentyfikował aż 3679 komisji, w których wystąpiły anomalie (a w mniej konserwatywnej wersji nawet 5453). Już te liczby budzą wątpliwości, bo ta druga stanowi 17% wszystkich analizowanych komisji, więc ciężko tu mówić o “anomaliach”. W modelowaniu statystycznym takie liczby świadczą zwykle o problemach z modelem (przyjęcie błędnych założeń). Model, który zwraca tak dużo anomalii, w pewnym sensie nie może być modelem typującym anomalie.

Skąd tak duże liczby? Autor przyjął tak liberalne kryteria, że nawet lekkie niepodobieństwo skutkuje zakwalifikowaniem wyników w danej komisji jako anomalii, co postaram się wykazać.

### 1

Grupy komisji zostały tak zdefiniowane, że ich wielkość w większości przypadków zawiera się w przedziale 10-16 (są to więc małe grupy). Dla każdej komisji jest liczona pewna cecha, np. "poparcie dla Nawrockiego". Jeśli to poparcie jest "nadmiernie" większe, niż w bliskich komisjach, jest to anomalia.
...
Pierwszy problem polega na tym, że ta reguła nie działa dla MAD. Należy przynajmniej uwzględnić, że np. dla rozkładu normalnego MAD jest ok. [1,5 razy mniejsze od SD](https://en.wikipedia.org/wiki/Median_absolute_deviation#Relation_to_standard_deviation){target="_blank"}. Innymi słowy, rzekomo konserwatywne podejście z progiem 3 wcale takie nie jest, bo de facto jest on równy 2. Takich "odstających" obserwacji jest w rozkładzie normalnym ok. 2%, co daje prawie 700 komisji uznanych za anomalie, mimo że oczywiście takimi nie są. Co więcej, w rzeczywistości rozkład wyników na pewno nie jest normalny, a np. w rozkładzie lognormalnym takich rzekomo odstających komisji będzie 21,5%, czyli prawie 7000.

Co prawda w pewnych sytuacjach rozkład może być taki, że obserwacji powyżej ustalonego powinno być mniej, w pracy nie ma jednak żadnego uzasadnienia, że mamy do czynienia z taką sytuacją. Autor po prostu przyjmuje sobie pewne liczby (najprawdopodobniej błędnie łącząc je z regułą trzech sigm) i w ogóle tego nie komentuje, co jest niedopuszczalne.
...
Drugi problem polega na tym, że nawet jeśli przyjąć bardziej rygorystyczne podejście (większy próg niż 3), mamy tu do czynienia z tzw. [problemem porównań wielokrotnych](https://danetyka.com/problem-wielokrotnego-testowania/){target="_blank"}. Z tego właśnie powodu w modelu, który sam zaproponowałem w poprzedniej części, zastosowałem poprawkę na wielokrotne testowanie. Nieuwzględnienie tego problemu jest błędem i autor popełnia go podwójnie. Po pierwsze, dokonuje ponad 30 tysięcy porównań (każda komisja), nie uwzględniając tego faktu w przyjętym progu. Po drugie, sprawdza niezależnie cztery cechy mające świadczyć o anomalii i ostatecznie raportuje wszystkie przypadki, dla których choć jedna z nich miałaby świadczyć o anomalii (tzn. w pracy jest to rozbite, ale wnioski są wyciąganie na podstawie sumy).

### 3

Spójrzmy jeszcze, jakie cechy bada autor. Jedna z nich jest nazwana "flip" i nie ma nic wspólnego z wyżej podaną standaryzacją i progami. Autor za anomalię uznaje każdy przypadek, w którym "Nawrocki wygrywa lokalnie, mimo że mediana wyników w grupie wskazuje przewagę Trzaskowskiego". Szczerze mówiąc, ciężko to skomentować. Załóżmy, że procenty poparcia dla Nawrockiego w danej grupie wynoszą: 45, 46, 47, 48, 49, 51, 52, 53, 54. Mediana wynosi 49%, więc "mediana wyników w grupie wskazuje przewagę Trzaskowskiego", a zatem te cztery (!) komisje, w których Nawrockich otrzymał ponad 50% to anomalie.

Oczywiście gdyby za anomalie traktować komisje, dla których np. wartości co najmniej trzech cech są odstające, to wtedy uwzględnienie powyższej miałoby sens. Ale jak pisałem, wystarczy jedna. I ta opisywana jest odpowiedzialna za ponad połowę (!) wskazań.

Dodam, że algorytm wyznacza jedynie anomalie na korzyść Nawrockiego. Nie ma żadnej wzmianki o tym, jakie wyniki otrzymamy, gdyby przeprowadzić podobną analizę na korzyść Trzaskowskiego. Ogólnie nie wiemy, jak powinien wyglądać rozkład liczonych statystyk przy założeniu, że w wyborach nie było żadnych fałszerstw (może właśnie tak, jak wygląda).

Podsumowując, praca autora miałaby sens tylko w przypadku, gdybyśmy wiedzieli, że wybory zostały sfałszowane na masową skalę (tzn. w bardzo dużej liczbie komisji podano nieprawdziwe wyniki). Wtedy zaprezentowany algorytm mógłby pomóc we wskazaniu komisji, w których miało to miejsce. Ale przecież to jest teza, którą autor chce wykazać.
...
* [Dr. Krzysztof Kontek](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5296441){target="_blank"}
